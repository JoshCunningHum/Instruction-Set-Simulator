class Instruction{
    ...
    constructor(
        ...
        this.address = Utils.padd(Utils.dec2Bin(Instruction.set.length), config.opcodeWidth);
    ...
    getMCBIN(...operand){
        ...
        for(let i = 0; i < 3; i++){
            ...
            if(!val){
                ...
                // for registers
                res = res.replace(`${i + 1}`, Register.get(val).address);
            ...
            let repl = Utils.dec2BinP(parseInt(val), log);
            if(config.sup_neg_num && val < 0) repl = Utils.comp(repl);
            // console.log(`${i + 1}: VAL : ${repl} : LOG : ${log}`);
            res = res.replace( `${i + 1}`, repl);
            count += repl.length;

        }
        res = res.replace("A", this.address);
        count += config.opcodeWidth;
        // add "0" to remaining
        let diff = MEM.romBit - count;
        if(diff < 0) diff = 0;
        // console.log("DIFF: ", diff);
        res = res.replace("-", "0".repeat(diff));
        return `${res}`;
    }
